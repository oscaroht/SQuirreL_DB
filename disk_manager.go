...
func NewDiskManager() *DiskManager {
	pm := map[PageID]Offset{}
	return &DiskManager{PageMap: pm, maxOffset: 0} // offset is zero before we read a file
}
...
func (d *DiskManager) readPage(pid PageID) (*Page, error) {
	...
	n2, err := f.ReadAt(buffer, int64(offset)) // call ReadAt instead of Read because after the Write action we are at the EOF. ReadAt(.., 0) specifies we need to move to the beginning
	if err != nil {
	...
	return deserializePage(buffer), &DiskError{}
...
func (d *DiskManager) writePage(p *Page) {
	...
	n, err := f.WriteAt(b, int64(p.PageOffset))
	if err != nil {
		slog.Error("Error", "message", err)
		panic(err)
	}
...
func (d *DiskManager) createNewPage() uint16 {
	offset := d.maxOffset + PAGE_SIZE
	d.maxOffset += PAGE_SIZE
